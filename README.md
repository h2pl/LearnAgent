...
2025-2026 年，AI Agent 相关岗位需求快速增长。大厂和创业公司都在招 Agent 开发、应用和架构方向的人才，面试也从「什么是 Agent」走向更深入的考察——ReAct 循环、工具调用设计、RAG 管线构建、多 Agent 协作编排，覆盖了从原理到工程的完整知识栈。

这是一套从零搭建 AI Agent 知识体系的完整路径。16 章从底层原理到工程交付逐层递进，每章解决一个核心问题，前后衔接形成闭环。不同于某个框架的教程或单个 demo 的展示，每一层都讲清楚为什么，而不只是怎么做。无论准备面试还是系统提升，这份路线图都能帮你查漏补缺。
