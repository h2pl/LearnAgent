...
你好，我是江小湖。从 [工具调用](../05-tool-use/01-tool-calling-mechanism.md) 到 [框架编排](../10-framework/01-framework-overview.md)，你的 Agent 已经具备了完整的执行能力。但这里有一个隐性问题：所有工具和交互都绑定在特定的模型和实现上——换个客户端就得重写集成，换个厂商就得重新适配。
