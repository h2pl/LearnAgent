...
你好，我是江小湖。在 [Agent 核心循环](../05-agent-loop/02-agent-core-loop.md) 中，你了解了 Agent 的"观察→决策→行动→观察"循环。但那个循环有一个致命问题：**每次会话都从零开始**。用户昨天告诉 Agent"我是 Java 开发者"，今天 Agent 又给你推荐 Python 代码了。这篇文章解决核心问题：**Agent 怎么在不同时间尺度上记住信息，让每次交互都比上一次更聪明**。
...
**短期记忆的核心挑战是容量**。上下文窗口有上限（GPT-4.1 是 1M tokens，Claude 3.5 是 200K tokens），一旦对话太长，早期的消息就会被截断。这就是 [08 — 上下文工程](../08-context-engineering/README.md) 要解决的问题。
