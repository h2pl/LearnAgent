...
你好，我是江湖哥。学完本章四篇文章，你已经掌握了 Prompt 工程的完整工具箱。这篇文章帮你把所有知识点串起来——不是简单罗列，而是展示它们之间的递进关系、适用场景和组合方式。
...
| 文章 | 解决的问题 | 核心交付 |
|------|-----------|---------|
| [01 — Prompt 设计模式](./01-prompt-design-patterns.md) | 不同场景下用什么方式组织 Prompt | 四种模式的选型决策树 |
| [02 — 结构化输出](./02-structured-output.md) | 如何让 LLM 输出可被代码消费的数据 | JSON Mode + Schema + 解析三层体系 |
| [03 — System Prompt 设计](./03-system-prompt.md) | 如何定义 Agent 的角色、边界和规范 | 四段式结构 + XML 标签组织法 |
| [04 — Prompt 鲁棒性](./04-prompt-robustness.md) | 如何在意外输入下保持安全可控 | 幻觉控制 + 边界处理 + 测试体系 |
...
**路径一：快速上手（2 小时）**
1. 读 [01 设计模式](./01-prompt-design-patterns.md) —— 掌握四种模式就够了
2. 读 [02 结构化输出](./02-structured-output.md) 的前半部分 —— JSON Mode 和 Schema 约束
...
- **[设计模式](./01-prompt-design-patterns.md)** 解决"怎么组织输入"——Zero-shot → Few-shot → CoT → 角色设定，从低控制到高控制的递进
- **[结构化输出](./02-structured-output.md)** 解决"输出格式"——JSON Mode 做格式保证，Schema 做结构定义，解析器做异常兜底
- **[System Prompt](./03-system-prompt.md)** 解决"行为定义"——四段式结构（角色/规则/边界/输出）+ XML 标签组织法
- **[鲁棒性](./04-prompt-robustness.md)** 解决"怎么稳"——幻觉控制、边界处理、一致性验证三层防御 + 测试体系
