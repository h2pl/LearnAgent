...
你好，我是江小湖。在 [停止条件设计](./04-stop-conditions.md) 中，你学会了让 Agent 安全停止。现在是时候把所有知识串起来——**从零实现一个最小 Agent**，不依赖 LangChain 或任何框架，纯代码实现 ReAct 循环。
...
最小 Agent 只是起点，生产系统需要补充：

| 缺失组件 | 说明 | 实现建议 |
|----------|------|----------|
| **错误处理** | 工具调用失败、LLM 超时 | 添加重试和降级逻辑 |
| **流式输出** | 实时显示 Agent 思考过程 | 使用 LLM 流式 API |
| **记忆持久化** | 保存对话历史到数据库 | 添加 SQLite/Redis 存储 |
| **权限控制** | 限制工具调用权限 | 添加参数校验和审计日志 |
| **多工具编排** | 支持并行工具调用 | 添加异步执行逻辑 |

**关键洞察**：框架（如 LangChain）封装了这些生产级功能，但会增加复杂度。理解最小 Agent 的本质，才能在需要时自己实现，不需要时用框架。
