...
很多读者一上来看到 5 个维度、几十种变体，第一反应是："这些不都是微调出来的吗？"**不是**。这 5 个维度的变体，来源完全不同。先搞清楚来源，再看细节，就不会晕。
...
| 维度 | 典型变体 | 训练方式 | 一句话说明 |
|------|---------|---------|-----------|
| **维度1：训练阶段** | Instruct / Chat / Reasoning | SFT → RLHF/DPO | 教模型"怎么回答"和"什么不该回答" |
| **维度2：能力专精** | Code / Math / Vision 专用 | 在通用模型基础上做领域 SFT + 对齐 | 用代码/数学/多模态数据替换通用对话数据，重新训练后几层 |
| **维度5：行业场景** | 金融 / 医疗 / 法律模型 | 通用模型 + 行业数据 SFT | 注入行业知识，同时保持通用能力 |
...
| 维度 | 典型变体 | 技术手段 | 一句话说明 |
|------|---------|---------|-----------|
| **维度3：部署形态** | 量化（Q4_K_M） | 把 FP16 权重映射到 4-bit 整数 | 像把 WAV 音乐压缩成 MP3——内容一样，精度降低 |
| **维度3：部署形态** | 合并（Merge） | 两个模型权重做线性插值 | 像把两杯不同口味的咖啡倒在一起 |
| **维度3：部署形态** | 剪枝（Pruning） | 把接近 0 的权重直接删除 | 像修掉盆景里多余的枝叶 |
...
| 维度 | 典型变体 | 架构差异 | 一句话说明 |
|------|---------|---------|-----------|
| **维度4：专用家族** | Embedding（BGE-M3） | 双塔 Encoder，不是 Decoder | 把文本变成向量——和生成文本是完全不同的数学任务 |
| **维度4：专用家族** | Rerank（BGE-reranker） | 交叉编码器（Cross-Encoder） | 判断"问题 A 和文档 B 有多相关"——不是生成 |
| **维度4：专用家族** | STT（Whisper） | 编解码器（Encoder-Decoder） | 语音→文字，输入是音频频谱图 |
| **维度4：专用家族** | TTS（CosyVoice） | 声学模型 + 声码器 | 文字→语音，输出是波形或 mel 谱 |

**关键特征**：这些模型和 LLM 的关系，就像冰箱和汽车的关系——都叫"家电"和"交通工具"，但内部结构完全不同。你**不能**把 Llama 3 "微调"成 Embedding 模型，就像不能把汽车改装成冰箱。

### 一个容易混淆的特例：蒸馏

蒸馏（Distillation）在维度3里，但它的来源有点特殊——**既不是纯后训练，也不是纯后处理**：
...
```
后训练（Post-Training）     后处理（Post-Processing）     独立训练（Independent）
       ↓                            ↓                            ↓
  同架构+新数据训练          权重数学变换，不训练           不同架构从头训练
  ──────────────────────────────────────────────────────────────────────────
  Instruct/Chat           量化(Q4_K_M)/合并/剪枝        Embedding/Rerank
  Code专用/Math专用           不改变模型能力                STT/TTS
  行业模型                   只改变存储和计算方式            和LLM完全不同的架构
```

**记住这个顺序**：先问"这个变体是从哪来的"——是后训练、后处理、还是独立训练？——再看具体细节。下文 5 个维度会按这个框架展开。
