...
> **模型跑在哪里？** 这是另一个问题。大多数情况下你会先调云端 API 快速原型；如果数据不能出内网或成本太高，再考虑本地部署（详见 [本地部署实战](./05-local-deployment.md)）。无论模型跑在哪，调用方式几乎一样——都是发 HTTP 请求到一个端点。
...
- **OpenAI API 格式是事实标准**——云端（OpenAI/DeepSeek）和本地（Ollama/vLLM）都兼容，一套代码走天下
- **SDK 是生产首选**——`openai` 官方 SDK 封装了认证、重试、流式等细节，比裸 HTTP 省心得多
