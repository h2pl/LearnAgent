...
你好，我是江小湖。前面我们讲了视觉理解（看）和语音交互（听说），这篇进入"创造"维度——让模型生成图片和视频。

在[第 1 章](../01-multimodal-fundamentals/02-core-mechanisms.md)我们已经讲过，多模态 AI 有两条完全不同的技术路线。理解侧用的是 Transformer 架构，生成侧用的是扩散模型架构。两者从训练方式到推理过程都截然不同。这篇会系统讲清楚生成侧的完整机制，而不只是"够用就好"。

## 生成侧：一条完全不同的技术路线

在讲 API 之前，先把生成侧和理解侧的关系讲清楚。

[第 1 章](../01-multimodal-fundamentals/02-core-mechanisms.md)讲了理解侧的三个核心机制：编码、融合、对齐。这些机制服务于 Transformer 架构，目标是"把多模态输入变成文字输出"。
...
接下来我们先讲 API 接入（实际使用），再深入生成侧的内部机制。
...
前面讲了怎么用 API，现在进入生成侧的内部。和理解侧的"编码→融合→对齐"对应，生成侧也有自己的核心机制链条。我们同样用系统的方式讲清楚。
...
训练的目标就是让模型学会"反向过程"——给定一个带噪声的图像，预测出应该去掉多少噪声。
...
下一篇我们把所有模态串起来，讲多模态推理——当模型需要同时处理图像、文字、语音做综合判断时，事情变得更有意思了。

> 下一章：[多模态推理：跨模态综合理解](../04-multimodal-reasoning/01-cross-modal-reasoning.md)
