...
你好，我是江小湖。选好模型之后、动手写代码之前，你需要先认识 API 上的几个关键参数——它们直接决定模型输出的质量和成本。

> **注意**：本文讲的是**推理时**的调参（temperature、top_p 等），不影响模型能力本身。想要真正改变模型行为，需要微调（Fine-tuning），详见 [模型微调实战指南](./07-finetuning-guide.md)。
...
<p align="center">
  <img src="../../assets/03-model-access/temperature-distribution.png" alt="Temperature 对概率分布的影响" width="90%"/>
  <br/>
...
<p align="center">
  <img src="../../assets/03-model-access/top-p-sampling.png" alt="Top_p 核采样原理" width="90%"/>
  <br/>
...
> 理解了这些参数，你就可以在 API 调用中精确控制模型行为了。请前往 [LLM API 调用实战](./03-api-calling.md)。
