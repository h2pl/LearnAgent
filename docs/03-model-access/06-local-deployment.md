...
你好，我是江小湖。前面的文章讲了怎么通过 API 调用云端模型。对于绝大多数人来说，云端 API 就是最优解——便宜、省心、随时用最强模型。但如果你有数据不出内网等硬性需求，这篇文章教你怎么在本地跑模型。
...
> **⚠️ 先说结论：个人开发者大多不需要本地部署**
>
> 即使你有一张 RTX 5090（32GB 显存，两三万起步），也只能跑 34B Q4 量化模型——能力大致相当于 GPT-4 早期水平。而云端旗舰模型（GPT-5.5、Claude Opus 4.8）是数千亿美元参数级别的服务，个人硬件物理上跑不了。
>
> 除非你有明确的数据隐私合规要求、或者日调用量大到自建成本更低，否则直接用云端 API 是更明智的选择。

**核心权衡**：

| 维度 | 本地部署 | 云端 API |
|------|---------|----------|
| 数据隐私 | ✅ 数据不出服务器 | ❌ 数据发送给第三方 |
| 成本 | 高调用量时更省 | 低调用量时更便宜 |
| 延迟 | 无网络往返，更低 | 受网络影响 |
| **模型能力** | **❗ 受硬件天花板限制，个人电脑跑不了前沿模型** | **✅ 随时调用最强模型** |
| 运维 | 自己承担 GPU、电力、更新 | 零运维 |
| 硬件投入 | 需要一次性购买 GPU | 无 |
...
|------|------|------------|-------------|---------|
| RTX 5060 (16GB) | 16 GB | 7B OK | 13B OK | ~$400 |
| RTX 5070 Ti (16GB) | 16 GB | 7B OK | 13B OK | ~$750 |
| RTX 5090 (32GB) | 32 GB | 13B OK | 34B OK / 70B Q4 勉强 | ~$2,000+ |
| RTX 4090 (24GB) | 24 GB | 13B OK | 34B OK | ~$1,200（上一代） |
| RTX A6000 (48GB) | 48 GB | 34B OK | 70B OK | ~$6,500 |

**对大多数个人开发者来说**：RTX 5090（32GB）是 2026 年的"甜点"——能跑 34B Q4 量化模型，32GB 显存比上代 4090 多出三分之一，能力覆盖范围更广。预算有限的话，上一代 4090 仍然是不错的选择。

<p align="center">
  <img src="../../assets/03-model-access/gpu-model-fit.png" alt="GPU 显存与可运行模型尺寸对照" width="90%"/>
  <br/>
...
| 芯片 | 统一内存 | 推荐配置 | 可运行模型 |
|------|---------|---------|-----------|
| M4 Pro | 24-48 GB | 48 GB | 13B Q4 / 34B Q4 |
| M4/M5 Max | 64-128 GB | 64 GB+ | 34B Q4 / 70B Q4 |
| M4/M5 Ultra | 192-256 GB | 192 GB+ | 70B Q4 |
...
<p align="center"><img src="../../assets/03-model-access/deployment-decision.svg" alt="Ollama vs vLLM部署决策" width="90%"/><br/><em>图：Ollama vs vLLM 选型决策与关键指标对比</em></p>

## 总结

- **本地模型的核心问题是硬件天花板**：即使 RTX 5090（32GB）也只能跑 34B Q4，大约 GPT-4 早期水平；而云端旗舰是千亿参数级别，个人硬件物理上跑不了
- **本地部署的价值**在于数据隐私和离线可用，但**对个人开发者性价比极低**——花几万买显卡，得到的远不如每月几十块的 API
- **硬件选型的关键指标是显存**：RTX 5090（32GB）能跑 34B Q4 模型，是个人开发的甜点；Apple M4/M5 Ultra（192-256GB）能跑 70B Q4
