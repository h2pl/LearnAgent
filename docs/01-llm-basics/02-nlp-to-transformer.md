...
[认识大语言模型（LLM）](./01-llm-overview.md)我们知道了 LLM 的本质是"预测下一个词"。但 LLM 不是凭空出现的——在它之前，人类已经花了几十年教机器"读懂文字"。**这篇文章回答的问题是：LLM 之前的技术到底卡在哪里？Transformer 又是怎么解决这些问题的？** 理解这段演进，你才能真正明白 LLM 为什么是今天这个样子，以及为什么 Transformer 是一切的基础。
...
这篇文章回顾了 NLP 的四个技术时代，每个时代都解决了一些问题，又留下了新的瓶颈：

- **规则时代**：靠手写规则处理语言，精确但脆弱，规则数量爆炸后无法维护。
- **统计学习时代**：用概率模型从数据中学习，不再依赖人工规则，但特征工程仍然靠人。
- **深度学习时代**：让神经网络自动学习特征，词向量带来了语义感知，但 RNN 的串行处理导致无法并行、长程遗忘。
- **Transformer**：自注意力机制一次性解决了长程依赖、并行化、任务通用性三个核心瓶颈，成为后续所有 LLM 的基础架构。
