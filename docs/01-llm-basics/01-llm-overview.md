...
1. **文本理解与生成**：这是 LLM 的本职工作。摘要、翻译、润色、扩写、风格改写，它都能胜任，而且质量远超传统 NLP 方法。
2. **代码生成与分析**：LLM 在海量 GitHub 代码上训练过，能熟练编写各种语言的代码、定位 Bug、写单元测试。这是目前 LLM 创造商业价值最大的领域之一。
3. **逻辑推理**：特别是最新的推理模型（如 o 系列、DeepSeek-R1），通过"先思考再回答"的机制，在数学、逻辑、规划等任务上有了质的飞跃。
4. **上下文学习（In-context Learning）**：不需要重新训练模型，只要在 Prompt 里给几个示例，它就能学会新的输出格式或处理逻辑。这是 LLM 灵活性的根源。
