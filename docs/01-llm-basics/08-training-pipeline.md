...
你好，我是江小湖。[LLM 发展简史](./03-llm-evolution.md)里我们知道了预训练→SFT→对齐这条时间线是怎么走过来的。但这三个阶段内部到底在做什么？为什么 Base 模型不听指令？为什么微调注入知识不如 RAG？这篇拆开看细节。
...
**目标**：让模型学会语言的规律和世界的知识。
**方法**：自监督学习——[认识大语言模型（LLM）](./01-llm-overview.md)讲过的"预测下一个词"循环，只是规模大到难以想象。
