...
<p align="center">
  <img src="../../assets/04-tool-use/parallel-vs-serial.svg" alt="并行调用vs串行调用：无依赖任务并行执行，依赖任务按序接力" width="95%"/>
</p>
...
**模型驱动链**的实现是一个循环：调用模型 → 检查是否返回工具调用 → 执行 → 回传 → 重复。这个循环的完整架构在 [05 — Agent 循环](../05-agent-loop/README.md) 中展开。核心代码骨架如下：
...
**为什么上下文膨胀还影响准确率**：自注意力机制对上下文中的所有 Token 计算关联度。上下文越长，注意力权重越分散——与当前决策相关的关键信号（最新的用户请求和工具结果）被大量历史 Token 稀释。这就是"迷失在中间"现象（[LLM 的局限与工程对策](../01-llm-basics/05-limitations.md)中讲过）在多工具场景中的具体表现。
