...
你好，我是江小湖。在 [Prompt 工程](../03-prompt-engineering/README.md) 中，你了解了如何精确控制 LLM 的输出。但如果 Agent 只能生成文本，它的能力就被关在笼子里——无法查天气、无法调 API、无法操作数据库。这篇文章解决核心问题：**工具调用如何让 LLM 与外部世界交互，执行真实操作**。
...
理解了这个概率机制，下一步就是看一次完整的工具调用怎么发生——从定义工具到结果回传。
...
模型收到用户请求后，在自回归生成过程中判断是否需要调用工具。这个判断不是"思考"的结果，而是 SFT 训练建立的**条件概率映射**在起作用——用户输入的语义与 Schema 描述文本匹配，概率最高的工具被选中输出。具体的架构机制参见 [工具选择的架构基础](#工具选择的架构基础)。
