...
你好，我是江小湖。前三篇讲了 Prompt 设计模式、结构化输出、System Prompt。这些都是在"理想情况"下工作的。但现实是：用户会输错、输乱、甚至故意输入恶意内容来探你的边界。Prompt 鲁棒性，就是让你的 Agent 在这些意外面前不崩溃、不乱说、不犯傻。
...
<p align="center">
  <img src="../../assets/04-prompt-engineering/prompt-injection-defense.png" alt="Prompt 注入防御体系" width="90%"/>
  <br/>
...
<p align="center">
  <img src="../../assets/04-prompt-engineering/prompt-engineering-workflow.png" alt="鲁棒性测试流程" width="90%"/>
  <br/>
...
- **幻觉控制**：引用约束 + 不确定性分级——让模型在不确定时输出'I don't know'而非编造
...
到这里，Prompt 工程的核心技术你已经全部学完——设计模式、结构化输出、System Prompt、鲁棒性，四层防御体系已经搭建完成。

> 提示工程解决了"怎么说"，接下来要解决"怎么做"。下一章进入 Agent 开发的核心—— [工具调用（Function Calling）](../05-tool-use/README.md)，让 LLM 调用外部函数和 API。
